<template>
    <view class="order-create">
        <scroll-view scroll-y class="viewport">
            <!-- 收货地址 -->
            <navigator class="shipment" hover-class="none" url="/pages/my/address/index?from=order" v-if="selectedAddress">
                <view class="user">
            ...
            <!-- 没有收货地址 -->
            <navigator class="shipment" hover-class="none" url="/pages/my/address/index?from=order" v-else>
                <view class="address"> 请选择收货地址 </view>
                <text class="icon icon-right"></text>
            </navigator>
            <!-- 商品信息 -->
            <view class="goods">
                <navigator v-for="item in orderPre.goods" :key="item.skuId" :url="`/pages/goods/index?id=${item.id}`"
...
export default {
    ...
    onShow() {
        this.getMemberOrderPre()
    },
    methods: {
        // 获取订单数据
        async getMemberOrderPre() {
            const { result } = await getMemberOrderPre()
            console.log(result);
...
.shipment {
    ...
    font-size: 26rpx;
    border-radius: 10rpx;
    background: url(https://static.botue.com/erabbit/static/images/locate.png) 20rpx center / 50rpx no-repeat #fff;
    position: relative;
}
