<template>
    <view class="order-detail">
        ...
        <scroll-view class="viewport" scroll-y>
            ...
            <view class="goods">
                <view class="item">
                    <navigator v-for="item in order.skus" :key="item.id" :url="`/pages/goods/index?id=${item.spuId}`"
                        hover-class="none">
...
import { getMemberOrderDetail } from '@/api/order'
...
export default {
    ...
    computed: {
        ...mapState(["safeArea", "platform", "capsule"]),
        countdown() {
            if (this.order) {
